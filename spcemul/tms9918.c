...
extern unsigned char *video_get_vbp(int line);
...
{
	return vdp->regs[7] & 0xf;
}

unsigned char *video_get_vbp(int line)
{
	return fbaddr + (width - 256)/2 + width * ((height - 192)/2 + line);
}
...
/* 	    deb_printf("tms9918: register write %d = %02x.\n", data & 7, vdp->addrsave); */
	    vdp->regs[data & 0x3f] = vdp->addrsave;
	} else {
...
int tms9918_cache_sprites(tms9918 vdp, struct sprite_cache *cache, int sprite_size, int mag)
{
    struct sprite_data *sprites;
    unsigned char *sprite_generator_table;
    int num_sprites;
    int i, y;
    ...
    sprite_generator_table = &vdp->memory[(vdp->regs[6] & 0x07) << 11];
    ...
    for (i = 0; i < 32; i++) {
		...
		if ((sprites[i].y_pos + (sprite_size<<mag)) < vdp->scanline) {
			continue;
		}
		if (num_sprites == 4) {
...
//			break;
		...
		y = (vdp->scanline - (sprites[i].y_pos + 1));
		...
		cache[num_sprites].pattern = &sprite_generator_table[sprites[i].pattern * 8] + (y >> mag);
		num_sprites++;
    }
...
void tms9918_draw_sprites(tms9918 vdp)
{
    struct sprite_data *sprites;
    unsigned char *sprite_generator_table;
    ...
    int num_sprites;
    int i, y, j, pitch;
    int sprite_size = (vdp->regs[1] & 0x02)? 16: 8;
    num_sprites = 0;
    sprites = (struct sprite_data *)&vdp->memory[(vdp->regs[5] & 0x7f) << 7];
    sprite_generator_table = &vdp->memory[(vdp->regs[6] & 0x07) * 0x800];
	for (i = 0; i < 32; i++) 
	{
		if (sprites[i].y_pos >= vdp->scanline) {
			continue;
		}
		pitch = video_get_vbp(1) - video_get_vbp(0); 
		screen = video_get_vbp(sprites[i].y_pos) + sprites[i].x_pos;
		screen2 = screen + pitch * sprite_size;
		for(j = 0; j < sprite_size; j++)
		{
			if (sprites[i].y_pos + j >= vdp->scanline)
				break;
			screen[j] = sprites[i].color;
			screen[pitch*j] = sprites[i].color;
			screen[pitch*j+(sprite_size-1)] = sprites[i].color;
			if ((sprites[i].y_pos + (sprite_size-1)) < vdp->scanline)
				screen2[j] = sprites[i].color;
		}			
	}
}
...
{
    struct sprite_cache cache[32];
    int num_sprites;
    int i;
    int sprite_size;

    sprite_size = (vdp->regs[1] & 0x02)? 16: 8;
	char mag = (vdp->regs[1] & 0x01);
    
    num_sprites = tms9918_cache_sprites(vdp, cache, sprite_size, mag);
	...
	if (!cache[i].color) {
	    /* no point in drawing an invisible sprite *tms9918_update1 */
	    continue;
	}
...
void tms9918_render_line_mode_0(tms9918 vdp)
{
    ...
    for (i = 0; i < 32; i++) {
		...
		for (j = 0; j < 8; j++)
			*cur_vbp++ = (pattern & (0x80 >> j) ? color1 : color0);
    }
}
...
void tms9918_render_line_mode_1(tms9918 vdp)
{
    ...
    cur_vbp = video_get_vbp(vdp->scanline)+8;
    nametable = vdp->memory + ((vdp->regs[2] & 0x0f) << 10);
...
void tms9918_render_line_mode_3(tms9918 vdp)
{
    ...
    for (i = 0; i < 32; i++) {
		...
		*cur_vbp++ = *cur_vbp++ = *cur_vbp++ = *cur_vbp++ = color1;
		*cur_vbp++ = *cur_vbp++ = *cur_vbp++ = *cur_vbp++ = color0;
    }
}
...
tms9918 tms9918_create(void)
{
    ...
    retval = (tms9918)malloc(sizeof(struct _tms9918)*2);
    if (retval) {
