...
int  rom_start = 0;		/* start address of ROM, 0 if none */
int  rom_boot = 0;		/* boot address in ROM */

/*
 *	Variables for memory of the emulated CPU
 */
#if 0
BYTE prot1[1024];		/* fence, for debugging */
#endif

BYTE ram[65536];		/* 64KB RAM */

#if 0
BYTE prot2[1024];		/* fence, for debugging */
#endif
...
#endif
char xfn[4096];			/* buffer for filename (options -x & -b) */
