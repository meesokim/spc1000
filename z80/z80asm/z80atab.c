...
struct opc *search_op(char *op_name)
{
	...
	i = 0;
	while((ch = op_name[i]) != 0)
		op_name[i++] = ch >= 'a' && ch <= 'z'  ? ch - 'a' + 'A' : ch;
	low = &opctab[0];
...
void put_undsym(char *s)
{
/*
	if (usym==0) {
		undsym = (char **) calloc(HASHSIZE * sizeof(char *), 0);
		printf("undsym(%d) created.\n", sizeof(char *));
	}
*/	
	int i = 0;
	for (i = 0; i < usym; i++)
		if (strcmp(undsym[i], s) == 0)
			return;
	undsym[usym] = calloc(strlen(s)+1,0);
	strcpy(undsym[usym], s);
//	printf("%d. %s, %c-%x\n", usym, undsym[usym], undsym[usym][0], (unsigned int)undsym[usym]);
//	printf("%x\n", (unsigned int)undsym[usym]);
	usym++;
}
