...
void asmerr(int i)
{
	if (pass == 1) {
		...
		fprintf(errfp, "%s: %s", errmsg[i], line);
		fprintf(errfp, "\n\n");
	} else
		errnum = i;
	errors++;
...
{
	fprintf(lstfp, ";Z80-Assembler\t\tRelease %s\t\t\t\tPage %d\n", REL,
		++page);
...
void lst_attl(void)
{
//	fprintf(lstfp, "\nSOURCE CODE 	LOC   OBJECT CODE   LINE   STMT \n");
...
void lst_undsym()
{
	...
	fprintf(lstfp, "\n\nTitle:       Undefined Symbol Table\n");
	fprintf(lstfp, "==========================================\n");
	for(i=0; undsym[i]>0;i++)
	{
		fprintf(lstfp, "%d. %s\n", i, undsym[i]);
		//printf("%d. %s %c %x\n", i+1,undsym[i], *(undsym[i]+1), undsym[i]);
		//fprintf(lstfp, "%s\n", undsym[i]);
	}
	fprintf(lstfp, "==========================================\n");
}
