...
static void ini(z80* const z) {
  uint8_t val = z->port_in(z->c | (z->b << 8));
  wb(z, get_hl(z), val);
  set_hl(z, get_hl(z) + 1);
  z->b -= 1;
  z->zf = z->b == 0;
...
static void outi(z80* const z) {
  z->port_out(z->c | (z->b << 8), rb(z, get_hl(z)));
  set_hl(z, get_hl(z) + 1);
  z->b -= 1;
  z->zf = z->b == 0;
  z->nf = 1;
...
void exec_opcode(z80* const z, uint8_t opcode) {
  ...
  case 0xDB: {
    ...
    const uint8_t a = z->a;
    z->a = z->port_in(port | (a << 8));
    z->mem_ptr = (a << 8) | (z->a + 1);
  } break; // in a,(n)

  case 0xD3: {
    const uint8_t port = nextb(z);
    z->port_out(port | (z->a << 8), z->a);
...
void exec_opcode_ed(z80* const z, uint8_t opcode) {
  ...
  case 0x41: z->port_out(z->c | (z->b << 8), z->b); break; // out (c), b
  case 0x49: z->port_out(z->c | (z->b << 8), z->c); break; // out (c), c
  case 0x51: z->port_out(z->c | (z->b << 8), z->d); break; // out (c), d
  case 0x59: z->port_out(z->c | (z->b << 8), z->e); break; // out (c), e
  case 0x61: z->port_out(z->c | (z->b << 8), z->h); break; // out (c), h
  case 0x69: z->port_out(z->c | (z->b << 8), z->l); break; // out (c), l
  case 0x71: z->port_out(z->c | (z->b << 8), 0); break; // out (c), 0
  case 0x79:
    z->port_out(z->c | (z->b << 8), z->a);
    z->mem_ptr = get_bc(z) + 1;
    break; // out (c), a
