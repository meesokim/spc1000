...
#elif RASPPI <= 3
#define PERIPHERAL_BASE     0x3F000000 // Peripheral Base Address
#else
...
#define PCM_FSLEN_SHIFT 0
#define PCM_FLEN        0xFFC00 // PCM Mode: Frame Length
#define PCM_FLEN_SHIFT  10
...
struct _pcm {
    uint32_t cs;  // PCM Control & Status
    uint32_t fifo;  // PCM FIFO Data 
    uint32_t mode;  // PCM Mode 
    uint32_t rxc;   // PCM Receive Configuration 
    uint32_t txc;   // PCM Transmit Configuration 
    uint32_t dreq;  // PCM DMA Request Level 
    uint32_t inten; // PCM Interrupt Enables 
    uint32_t intstc;// PCM Interrupt Status & Clear 
...
__attribute__ ((naked)) void dsb();
__attribute__ ((naked)) void flush_cache();
__attribute__ ((naked)) void clean_cache();
...
extern volatile struct   _pcm * PCM;
