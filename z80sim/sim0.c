...
usage:
...
#ifdef HAS_DISKS
				printf("usage:\t%s -z -8 -s -l -i -u %s-mn -fn -xfilename -ddiskpath\n", pn, rom);
#else
				printf("usage:\t%s -z -8 -s -l -i -u %s-mn -fn -xfilename\n", pn, rom);
#endif
				...
				printf("\t     %s\n", BOOTROM);
#endif
				puts("\t-m = init memory with n (00-FF)");
...
puts(" #####    ###     #####    ###            #####    ###   #     #");
	...
	/* seed the random generator from some random device if possible */
