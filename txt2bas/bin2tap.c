...
int main(int argc, char *argv[])
{
	...
	if ( (argc<2) || (argc>5) ) {
		fprintf(stdout,"Usage: %s [bin/ihex file] [tap file]\n",argv[0]);
		fprintf(stdout,"   or: %s [bin file] [tap file] [start]\n",argv[0]);
		fprintf(stdout,"   or: %s [bin file] [tap file] [start] [execution]\n",argv[0]);
		exit(1);
	}

	if (argc < 3 || atoh(argv[2]) == -1)
	{
	...
	if (strncasecmp(argv[1] + strlen(argv[1]) - 3, "ihx", 3) == 0 || strncasecmp(argv[1] + strlen(argv[1]) - 5, "ihex", 4))
	{
		parseIHEX(fpin);
	}
	else
