...
void CIRCLEAUD_DmaInterruptHandler() {
    int16_t * src;
    uint32_t data;
...
static void CIRCLEAUD_CloseDevice(_THIS) {
...
static int CIRCLEAUD_OpenDevice(_THIS, const char *devname, int iscapture) {
...
static void CIRCLEAUD_LockDevice(_THIS) {
    locked++;
}

static void CIRCLEAUD_UnlockDevice(_THIS) {
    if (this->paused) {
        IRQ->irq1Disable = INTERRUPT_DMA0;
        DMA->ch[0].cs = DMA_RESET;
        running = 0;
...
static Uint8 * CIRCLEAUD_GetDeviceBuf(_THIS) {
    return audio_buffer;
}

static int CIRCLEAUD_Init(SDL_AudioDriverImpl * impl) {

    /* Set the function pointers */
    impl->OpenDevice = CIRCLEAUD_OpenDevice;
    impl->CloseDevice = CIRCLEAUD_CloseDevice;
    impl->LockDevice = CIRCLEAUD_LockDevice;
    impl->UnlockDevice = CIRCLEAUD_UnlockDevice;
    impl->GetDeviceBuf = CIRCLEAUD_GetDeviceBuf;
...
AudioBootStrap CIRCLEAUD_bootstrap = {
    "rpi", "SDL Raspberry Pi audio driver", CIRCLEAUD_Init, 0
};
