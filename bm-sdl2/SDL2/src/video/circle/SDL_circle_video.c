...
static int CIRCLE_VideoInit(_THIS);
static int CIRCLE_SetDisplayMode(_THIS, SDL_VideoDisplay * display, SDL_DisplayMode * mode);
static void CIRCLE_VideoQuit(_THIS);
static int CIRCLE_GetDisplayBounds (_THIS, SDL_VideoDisplay * display, SDL_Rect * rect);

static unsigned int phys_width;
static unsigned int phys_height;

static int
CIRCLE_CreateWindow(_THIS, SDL_Window * window)
{
...
static int
CIRCLE_Available(void)
{
    return (1);
}

static void
CIRCLE_DeleteDevice(SDL_VideoDevice * device)
{
    SDL_free(device);
}
...
#ifdef HAVE_USPI
static void
CIRCLE_USPiKeyStatusHandlerRaw (unsigned char ucModifiers, const unsigned char RawKeys[6]) {
    int i, index;
    unsigned char key;
...
CIRCLE_CreateDevice(int devindex)
{
    ...
    device->SetDisplayMode = CIRCLE_SetDisplayMode;
    device->PumpEvents = CIRCLE_PumpEvents;
    device->CreateWindow = CIRCLE_CreateWindow;
    ...
    device->GetDisplayBounds = CIRCLE_GetDisplayBounds;
    ...
    if (USPiKeyboardAvailable()) {
        USPiKeyboardRegisterKeyStatusHandlerRaw (CIRCLE_USPiKeyStatusHandlerRaw);
    }
#endif
...
VideoBootStrap CIRCLE_bootstrap = {
    CIRCLEVID_DRIVER_NAME, "SDL Raspberry Pi video driver",
    CIRCLE_Available, CIRCLE_CreateDevice
};

int
CIRCLE_VideoInit(_THIS)
{
...
static int
CIRCLE_SetDisplayMode(_THIS, SDL_VideoDisplay * display, SDL_DisplayMode * mode)
{
    phys_width = mode->w;
    phys_height = mode->h;
    return 0;
...
void
CIRCLE_VideoQuit(_THIS)
{
}

static int
CIRCLE_GetDisplayBounds (_THIS, SDL_VideoDisplay * display, SDL_Rect * rect)
{
    rect->x = 0;
    rect->y = 0;
    rect->w = phys_width;
